...
RSpec.describe TextAdventures::CharacterProgression do
  ...
  it "uses the configured quadratic level curve" do
    expect(described_class.xp_required_for(1)).to eq 50
    expect(described_class.xp_required_for(2)).to eq 200
    expect(described_class.xp_required_for(3)).to eq 450

    progression.add_skill_xp(:combat_magic, 199)
